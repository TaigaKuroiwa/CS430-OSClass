...
/*
 * Tests in this file:
 * 	1.  Demo of read_sector and write_sector 1 
 * 	2.  Call to format (should not crash, etc) 1 
 * 	3.  Save and load a file (phrase.txt) 1
 * 	4.  Save a bigger (multi-sector) file x 
 * 	5.  Delete a file 1
 * 	6.  Try to load a file that does not exist 1
 * 	7.  Try to delete a file that does not exist 1
 * 	8.  Try to re-use an already-used filename 1 
 * 	9.  Fill the entire drive and check function of NO_SPACE 1
 * 	10. Try to read from full drive x
 * 	11. Delete everything in the drive and check save/load again x
 * 	12. Format and check to make sure all files are "gone" 0.5
 *
...
int main() {
	...
	if(ret = load("phrase.txt", buf, 1024))
		printf("** this is load Received error value:  %d\n", ret);
	...
	if(ret = fdelete("phrase.txt"))
		printf("** test5 Received error value:  %d\n", ret);
	printf("passed test5!\n");
	// Test 6
	...
	if(ret == load("longph.txt", longphrasecpy, strlen(longphrase)))
		...
		printf("longphrease: %s", longphrasecpy);
	free(longphrase);
	free(longphrasecpy);
